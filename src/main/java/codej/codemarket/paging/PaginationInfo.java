package codej.codemarket.paging;
...
public class PaginationInfo {
    ...
    private void calculation() {
         ...
         *  만약 전체 데이터 개수가 99개 일 때 페이지 개수는 98 / 10 갸묜 9.8 인데 0.8단위는 없으므로 +1 */

        totalPageCount = ((totalRecordCount - 1) / criteria.getRecordsPerPage()) + 1;
//        totalPageCount = (int) (Math.ceil(totalPageCount - 1) / criteria.getRecordsPerPage()) + 1; //<---이거 되는지 확인!
        // totalPageCount = totalRecordCount/criteria.getRecordPerPage + (totalRecordCount/criteria.getRecordPerPage >0 ? 1 : 0)
        if (criteria.getCurrentPageNo() > totalPageCount) {
            criteria.setCurrentPageNo(totalPageCount);
        }
        /**페이지 리스트의 첫 페이지 번호
         *  주의 ! int 형은 나누면 소숫점 버림.
         * */
